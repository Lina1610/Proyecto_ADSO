...
def actualizar_datos_usuario(user_id, nombre, apellido, documento, telefono=None):
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor() as cursor:
                query = """
                    ...
                    WHERE id = %s
                """
                valores = (nombre, apellido, documento, telefono, user_id)
                cursor.execute(query, valores)
                conexion_MySQLdb.commit()
                return cursor.rowcount > 0  # Devuelve True si se actualizó
    except Exception as e:
        print(f"Error en actualizar_datos_usuario: {str(e)}")
        return False
