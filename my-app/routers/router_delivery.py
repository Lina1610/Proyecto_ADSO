...
def viewEditarEntrega(id):
    ...
    if request.method == 'GET':
        entrega = buscar_entrega_por_id(id)
        if isinstance(entrega, dict):
            return render_template('public/entrega/editar_entrega.html', entrega=entrega, entrega_id=id)
        else:
            flash(entrega, 'error')
            return redirect(url_for('lista_entregas'))
        ...
        if error:
            flash(error, 'error')
            return render_template('public/entrega/editar_entrega.html', entrega=data_form, entrega_id=id)
