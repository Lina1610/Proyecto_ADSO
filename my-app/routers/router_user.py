...
def cambiar_contrasena():
    ...
    if nueva_password != repetir_password:
        return jsonify({'success': False, 'message': 'Las contraseñas no coinciden. Por favor, verifica.'})

    # Validar longitud de la contraseña
    if len(nueva_password) < 8 or len(nueva_password) > 20:
        return jsonify({'success': False, 'message': 'La contraseña debe tener entre 8 y 20 caracteres.'})

    # Validar que no sea solo letras
    if nueva_password.isalpha():
        return jsonify({'success': False, 'message': 'La contraseña no puede contener solo letras.'})

    # Validar que no sea solo números
    if nueva_password.isdigit():
        return jsonify({'success': False, 'message': 'La contraseña no puede contener solo números.'})

    # Validar que contenga al menos dos de los tres requisitos (letras, números, caracteres especiales)
    ...
    requirements_met = sum([has_letters, has_numbers, has_special])
    if requirements_met < 2:
        return jsonify({'success': False, 'message': 'La contraseña debe contener al menos dos de los siguientes: letras, números o caracteres especiales (por ejemplo, !@#$%^&*).'})

    # Actualizar la contraseña
    if actualizar_password(user_id, nueva_password):
        return jsonify({'success': True, 'message': 'Contraseña actualizada correctamente.', 'reload': True})
    return jsonify({'success': False, 'message': 'Error al actualizar la contraseña.'})
